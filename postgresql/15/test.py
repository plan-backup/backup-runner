...
class PostgreSQLIntegrationTest15(DatabaseTestFramework):
...
def main():
    ...
    logger.info("🚀 Starting postgresql 15 Integration Test")
    test = PostgreSQLIntegrationTest15()
    try:
        # Run the complete backup and restore test using the shared framework
        success = test.run_backup_and_restore_test()
        
        if success:
            logger.info("🎉 postgresql 15 integration test completed successfully!")
