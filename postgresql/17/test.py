...
class PostgreSQLIntegrationTest17(DatabaseTestFramework):
...
def main():
    ...
    logger.info("🚀 Starting postgresql 17 Integration Test")
    test = PostgreSQLIntegrationTest17()
    try:
        # Run the complete backup and restore test using the shared framework
        success = test.run_backup_and_restore_test()
        
        if success:
            logger.info("🎉 postgresql 17 integration test completed successfully!")
