...
class PostgreSQLIntegrationTest16(DatabaseTestFramework):
...
def main():
    ...
    logger.info("🚀 Starting postgresql 16 Integration Test")
    test = PostgreSQLIntegrationTest16()
    try:
        # Run the complete backup and restore test using the shared framework
        success = test.run_backup_and_restore_test()
        
        if success:
            logger.info("🎉 postgresql 16 integration test completed successfully!")
