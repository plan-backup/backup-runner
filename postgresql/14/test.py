...
class PostgreSQLIntegrationTest14(DatabaseTestFramework):
...
def main():
    ...
    logger.info("🚀 Starting postgresql 14 Integration Test")
    test = PostgreSQLIntegrationTest14()
    try:
        # Run the complete backup and restore test using the shared framework
        success = test.run_backup_and_restore_test()
        
        if success:
            logger.info("🎉 postgresql 14 integration test completed successfully!")
