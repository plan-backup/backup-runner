...
class MariaDBIntegrationTest1202:
    def __init__(self):
        """Initialize MariaDB 12.0.2 integration test"""
        ...
        self.container = None
        self.container_name = f"planb_test_mariadb_1202_{int(time.time())}"
    ...
    def start_database_container(self):
        """Start MariaDB 12.0.2 container for testing"""
        logger.info("🐬 Starting MariaDB 12.0.2 container...")
    ...
    def build_container(self):
        ...
        logger.info("🔨 Building MariaDB 12.0.2 backup container...")
        
        try:
            # Build the container
            image, build_logs = self.client.images.build(
                path="/Users/diablo/Projects/react/backup-runner",
                dockerfile="mariadb/12.0.2/Dockerfile",
                tag="gcr.io/apito-cms/plan-b-backup-mariadb:test-1202",
                rm=True
            )
            
            logger.info("✅ MariaDB 12.0.2 backup container built successfully")
            return True
    ...
    def run_backup_test(self):
        ...
        logger.info("🧪 Running MariaDB 12.0.2 backup test...")
        
        try:
            # Run the backup container
            backup_container = self.client.containers.run(
                "gcr.io/apito-cms/plan-b-backup-mariadb:test-1202",
                environment={
            ...
            if result['StatusCode'] == 0:
                logger.info("✅ MariaDB 12.0.2 backup test completed successfully")
    ...
    def push_to_gcr(self):
        ...
        logger.info("🚀 Pushing MariaDB 12.0.2 container to GCR...")
        ...
        try:
            ...
            image, build_logs = self.client.images.build(
                path="/Users/diablo/Projects/react/backup-runner",
                dockerfile="mariadb/12.0.2/Dockerfile",
                tag="gcr.io/apito-cms/plan-b-backup-mariadb:12.0.2",
            ...
            for line in self.client.images.push(
                "gcr.io/apito-cms/plan-b-backup-mariadb:12.0.2",
                stream=True,
                decode=True
            ):
                if 'status' in line:
            ...
            logger.info("✅ MariaDB 12.0.2 container pushed to GCR successfully")
            return True
    ...
    def run_full_test(self):
        """Run the complete MariaDB 12.0.2 integration test"""
        logger.info("🚀 Starting MariaDB 12.0.2 Build & Test Pipeline")
        logger.info("=" * 60)
        ...
        try:
            ...
            logger.info("🎉 MariaDB 12.0.2 integration test completed successfully!")
            return True
            
        except Exception as e:
            logger.error(f"❌ MariaDB 12.0.2 integration test failed: {e}")
            return False
        finally:
            self.cleanup()

if __name__ == '__main__':
    test = MariaDBIntegrationTest1202()
