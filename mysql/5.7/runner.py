#!/usr/bin/env python3
"""
Plan B Database Runner - MySQL 5.7
...
class MySQLRunner:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with open(backup_file_path, 'w') as f:
                    result = subprocess.run(mysqldump_cmd, stdout=f, stderr=subprocess.PIPE)
                
                if result.returncode != 0:
                    # Decode stderr bytes to string for Python 3.6 compatibility
                    stderr_text = result.stderr.decode('utf-8') if result.stderr else 'Unknown error'
                    raise Exception(f"mysqldump failed: {stderr_text}")
    ...
    def _restore_to_database(self, sql_file_path):
        ...
        try:
            ...
            result = subprocess.run(create_db_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
            ...
            with open(sql_file_path, 'r') as f:
                result = subprocess.run(restore_cmd, stdin=f, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
